# Апдейт для проверяющего по правкам ревью (все по замечаниям чисто сделал)
Задание по джанге
0. из настроек жанги выпилил импорт из __init__ из-за flake8, но чет не подумал что у меня перестанут подтягиваться настройки, проблему искал 2 дня))
1. в модельках добавил М2М связь с Person и связь с Genre и уникальность
2. DebugToolbarMiddleware добавил при дебаге
Задание по миграции
1. модельки вынес в отдельный модуль models_dataclass.py
2. налепил маппинги, чтобы код не повторялся и читался лучше, а в скрипте run_migration.py мне кажется добавлять дял 5 транкейтов таблиц маппинг и цикл не целесообразно и это лишнее усложнение, так что я не делал тут маппинг
